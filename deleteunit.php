...
if(!($stmt = $mysqli->prepare("DELETE FROM dzc_unit_weapon WHERE uid=" . $_POST['uid']))){
...
if(!($stmt = $mysqli->prepare("DELETE FROM dzc_unit WHERE id=" . $_POST['uid']))){
	echo "Prepare failed: "  . $stmt->errno . " " . $stmt->error;
}
if(!$stmt->execute()){
	echo "Execute failed: "  . $stmt->errno . " " . $stmt->error;
} else {
	echo "Deleted " . $stmt->affected_rows . " from dzc_unit.";
}
$stmt->close();
